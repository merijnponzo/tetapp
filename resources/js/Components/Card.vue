...
<template>
    <article
        class="h-[256px] bg-white relative rounded-xl p-4  text-4xl flex justify-center items-center hover:ring-blue-800 hover:ring-2 hover:text-blue-800">
        <p>{{ card.name }}</p>
        ...
        <template v-if="showSubcards">
            <div class="bg-black bg-opacity-15 fixed top-0 left-0 inset-0 flex justify-center items-center z-50"
                ...
                <div class="bg-white rounded-xl min-w-[80vw] h-[80vh] shadow-md relative" @click.stop>
                    <PrimaryButton @click="showSubcards = false, showSubcardActive = ''" class="absolute top-2 right-2">
                        <IconClose class="w-8 h-8" />
                    </PrimaryButton>
                    <template v-if="!showSubcardActive">
                        <div class="grid md:grid-cols-2 justify-between items-center p-4 gap-4 overflow-y-scroll">
                            <div v-for="subcard in subcards" @click="showSubcardActive = subcard"
                                class="bg-white rounded-xl w-full text-4xl ring-purple-30 min-h-[144px] ring-2 p-4 xl:p-8 text-center overflow-hidden">
                                {{ subcard }}
                            </div>
                        </div>
                    </template>
