...
const submit = () => {
    form.post(route('browse.form.create', { category_id: props.category_id, card_id: props.card_id }), {
        onFinish: () => form.reset(),
    });
};

const remove = () => {
    form.delete(route('browse.form.delete', { category_id: props.category_id, card_id: props.card_id }), {
        onFinish: () => form.reset(),
    });
};
        ...
        <div class="w-full flex justify-center py-8 flex-col items-center">
            <form @submit.prevent="submit" class="w-full md:w-[400px] max-w-[640px] bg-white p-4 rounded-xl shadow-xl">
                <div class="flex flex-col gap-2">
                    <h4 class="text-2xl">{{ category.name}}</h4>
